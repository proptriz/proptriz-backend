...
import UserService, { handleOtpAuth } from "../services/user.service";
...
const UserController = {
  ...
  async otpVerification(req: Request, res: Response) {
    try {
      ...
      if(!user) return res.status(404).json({ success: false, message: "OTP expired, Please Resend!!!" });
