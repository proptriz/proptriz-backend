...
//   try {
//     console.info(`Generating token for user: ${user.pi_uid}`);
//     const token = jwt.sign({ userId: user.pi_uid, _id: user._id }, env.JWT_SECRET, {
//       expiresIn: "1d", // 1 day
//     });
//     console.info(`Successfully generated token for user: ${user.pi_uid}`);
//     return token;
//   } catch (error) {
//     console.error(`Failed to generate user token for piUID ${ user.pi_uid }:`, error);
